...
function logPluginRequestError(gutil, action, err) {
  return logRequestError(gutil, action + ' plugin', err);
}

function logRequestError(gutil, action, err) {
  var msg = logError(gutil, 'Error making request to ' + action + '.');
    ...
    if (serviceResponse.hasOwnProperty('status')) {
      if (serviceResponse.status.indexOf('_FAIL') > -1) {
        ...
        if (serviceResponse.hasOwnProperty('hard-failures')) {
          hardFailures = serviceResponse['hard-failures'];
        } else if (serviceResponse.hasOwnProperty('hardFailures')) {
          hardFailures = serviceResponse['hardFailures'];
        }
        ...
        hardFailures.forEach(function (hardFailure) {
          ...
          if (typeof errorSuggestion === 'string' && errorSuggestion.trim().length > 0) {
...
module.exports = {
    ...
    logDebug: logDebug,
    logPluginRequestError: logPluginRequestError,
