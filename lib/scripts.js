...
var JS_MAIN_PATTERN = 'angular-li/src/!(activecast|react-li)/**/!(*.demo|*.spec|*.mock).js';
var JS_DEMO_PATTERN = 'angular-li/src/!(activecast|react-li)/**/*.demo.js';
var JS_MOCK_PATTERN = 'angular-li/src/!(activecast|react-li)/**/*.mock.js';
var JS_SPEC_PATTERN = 'angular-li/src/!(activecast|react-li)/**/*.spec.js';
var TPL_PATTERN = 'angular-li/src/!(activecast|react-li)/**/!(*.demo).tpl.html';
var TPL_DEMO_PATTERN = 'directives/**/*.demo.tpl.html';
var BASE = 'angular-li/src';
...
module.exports = function (gulp, gutil) {
  ...
  function hackFormatDep(dep) {
    return dep.replace(/^(angular-li\/bower_components|node_modules)/, '');
  }
  ...
  function _processTpls() {
    return through().obj(function (file, enc, cb) {
      ...
      if (file.path.match(/.*\/src\/services\/.*/)) {
         ...
         * the same name.
         */
        url = file.path.substr(path().join(file.cwd, 'angular-li', 'src').length + 1);
        moduleType = 'services';

      } else {
        url = file.path.substr(path().join(file.cwd, 'angular-li', 'src', 'directives').length + 1);
