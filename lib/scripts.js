...
module.exports = function (gulp, gutil) {
  const LIMUIRS_COMPONENT_PATH = gutil.env.newStructure ? 'limuirs/components/' : 'limuirs/components/';
  const COMPONENT_DEPS_SRC_PATH = gutil.env.newStructure ? 'limuirs/src/components' : 'limuirs/src/components';
  const COMPONENT_DEPS_DEST_PATH = gutil.env.newStructure ? 'dist/plugin/res/feature/component-dependencies.json' : 'plugin/res/feature/component-dependencies.json';
  var PLUGIN_SCRIPTS_PATH = gutil.env.newStructure ? 'dist/plugin/res/js/angularjs' : 'plugin/res/js/angularjs';
  var SCRIPTS_DEPS_PATH = gutil.env.newStructure ? 'dist/plugin/res/js/angularjs/lib' : 'plugin/res/js/angularjs/lib';
  var SCRIPTS_DEPS_METADATA_PATH = gutil.env.newStructure ? 'dist/plugin/res/js/angularjs/metadata' : 'plugin/res/js/angularjs/metadata';
  var SCRIPT_DEPENDENCIES_PATH = gutil.env.newStructure ? 'dist/plugin/res/js/angularjs/metadata' : 'plugin/res/js/angularjs/metadata';
  var JS_MAIN_PATTERN = gutil.env.newStructure ? 'angular-li/src/!(activecast|react-li)/**/!(*.demo|*.spec|*.mock).js' : 'src/!(activecast|react-li)/**/!(*.demo|*.spec|*.mock).js';
  var JS_DEMO_PATTERN = gutil.env.newStructure ? 'angular-li/src/!(activecast|react-li)/**/*.demo.js' : 'src/!(activecast|react-li)/**/*.demo.js';
  var JS_MOCK_PATTERN = gutil.env.newStructure ? 'angular-li/src/!(activecast|react-li)/**/*.mock.js' : 'src/!(activecast|react-li)/**/*.mock.js';
  var JS_SPEC_PATTERN = gutil.env.newStructure ? 'angular-li/src/!(activecast|react-li)/**/*.spec.js' : "src/!(activecast|react-li)/**/*.spec.js";
  var TPL_PATTERN = gutil.env.newStructure ? 'angular-li/src/!(activecast|react-li)/**/!(*.demo).tpl.html' : 'src/!(activecast|react-li)/**/!(*.demo).tpl.html';
  var TPL_DEMO_PATTERN = 'directives/**/*.demo.tpl.html';
  var BASE = gutil.env.newStructure ? 'angular-li/src' : 'src';
  ...
  /**
   * bundles scripts based on patterns and puts it into dest with the specified destFilename.
   */
  function processBundle(patterns, dest, destFilename, isWatch, isLint) {
    var fail = gutil.env['lint-fail'];
  ...
  function jscs(fail) {
    var pipe = lazypipe()();
    var jscsrcPath = gutil.env.newStructure ? 'angular-li/.jscsrc' : '.jscsrc'

    if (!fs.existsSync(jscsrcPath)) {
      return pipe.pipe(gutil.noop)();
    }
    pipe = pipe
      .pipe(gjscs(), fs.existsSync(jscsrcPath) ? { configPath: jscsrcPath } : {})
      .pipe(gjscs().reporter);
  ...
  function hackFormatDep(dep) {
    var regex = gutil.env.newStructure ? /^(angular-li\/bower_components|node_modules)/ : /^(bower_components|node_modules)/;
    return dep.replace(regex, '');
  }
  ...
  function createDepsMetadata(src, dest, isWatch) {
    var moduleFiles = (function () {
      ...
      Object.keys(gutil.env.ng.moduleDependencyMap || {}).forEach(function (key) {
        if (gutil.env.ng.useNewModuleDependenciesFormat === true) {
          ...
          if (Array.isArray(moduleDeps)) {
            moduleDeps.forEach(function (dep) {
              return moduleDevDependencyArray.push(path().join('lib', hackFormatDep(dep)));
            });
          }
          ...
          if ('min' in moduleDeps && Array.isArray(moduleDeps.min)) {
            moduleDeps.min.forEach(function (dep) {
              return moduleMinDependencyArray.push(path().join('lib', hackFormatDep(dep)));
            });
          }
          deps[key] = {
          ...
          deps[key] = gutil.env.ng.moduleDependencyMap[key].map(function (dep) {
            return path().join('lib', hackFormatDep(dep));
          });
        }
      });
        return deps;
  ...
  function _processTpls() {
    return through().obj(function (file, enc, cb) {
      ...
      var moduleType, url;
      var joinPath = gutil.env.newStructure ? "angular-li/src" : "src";
      if (file.path.match(/.*\/src\/services\/.*/)) {
         ...
         * the same name.
         */
        url = file.path.substr(path().join(file.cwd, joinPath).length + 1);
        moduleType = 'services';

      } else {
        url = file.path.substr(path().join(file.cwd, joinPath, 'directives').length + 1);
      ...
      file.base = path().join(file.cwd, joinPath);
  ...
  return {
    ...
    PLUGIN_SCRIPTS_PATH,
    SCRIPTS_DEPS_PATH,
    SCRIPT_DEPENDENCIES_PATH,
    SCRIPTS_DEPS_METADATA_PATH,
    BASE,
    LIMUIRS_COMPONENT_PATH,
    COMPONENT_DEPS_SRC_PATH,
    COMPONENT_DEPS_DEST_PATH
  };
};
