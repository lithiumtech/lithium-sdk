  ...
  function createDepsMetadata(src, dest, isWatch) {
    var moduleFiles = (function () {
      ...
      Object.keys(gutil.env.ng.moduleDependencyMap || {}).forEach(function (key) {
        var moduleDependencyArray = [];
        gutil.env.ng.moduleDependencyMap[key].dev.forEach(function (dep) {
          if (gutil.env.ng.moduleDependencyMap[key].min.indexOf(dep.replace('.js', '.min.js')) < 0) {
            return moduleDependencyArray.push(path().join('lib', dep));
          }
        });
        gutil.env.ng.moduleDependencyMap[key].min.forEach(function (dep) {
          return moduleDependencyArray.push(path().join('lib', dep));
        });
        deps[key] = moduleDependencyArray;
      });
      return deps;
    })();
