...
module.exports = function (gulp, gutil) {
  	...
  	function processResponse(req, pluginType, doClear, useVerboseMode) {
		...
		function writeZip(pluginType, res, buf) {
    		try {
                if (res.statusCode > 201) {
                    ...
                    writeError(serviceResponse, 'export-' + pluginType + '-plugin');
                } else {
                ...
                gutil.log(gutil.colors.red(pluginType + ' plugin write failed'));
		...
		req.on('response', function (res) {
            ...
            pluginUtils.logRequestError(gutil, doClear ? 'clear' : 'export', err);
            process.exit(1);
        });
  	}
