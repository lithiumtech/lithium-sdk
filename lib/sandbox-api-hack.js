...
module.exports = function (gulp, gutil) {
  var SUPPORTED_UPDATE_PLUGIN_POINTS = ['skins', 'quilts', 'layouts', 'lang', 'components'];
      ...
      ['-Rd']
        .concat(filesToUpload.map(
          function (filePath) {
            ...
            if (parts.length > 2 &&
                parts[0] === 'res') {
              if(SUPPORTED_UPDATE_PLUGIN_POINTS.indexOf(parts[1]) > -1) {
                reload[parts[1]] = true;
              } else if (SKIP_UPDATE_PLUGIN_POINTS.indexOf(parts[1]) === -1) {
                reload.all = true;
              }
            }
            return relativePath;
