...
module.exports = function (gulp, gutil) {
  ...
  function createReloadQuery(files) {
    var reload = {};
  ...
  function refreshPlugin(reload, callback) {
    var req = request({
      ...
      res.pipe(through(
        ...
        function (cb) {
          if (result && result.reloaded) {
            ...
            callback(new Error('Failed to reload plugin, please make sure your server.conf.json is configured' +
              'properly and that the "phase" config for your community is set to "dev"'));
          }
          cb();
        }
      ));
  ...
  function deleteFiles(cb) {
    spawn('rsync', [
      ...
      if (code === 0) {
        ...
        gutil.log(gutil.colors.red('Failed to reset sandbox plugin'));
      }
    });
  }

  function copyFiles(files, cb) {
    spawn('rsync', ['-r', files, server.sandboxPluginDir()], {
      cwd: './',
      stdio: ['pipe', 'ignore', 'inherit']
    }).on('close', function (code) {
      if (code === 0) {
  ...
  return {
    ...
    //uploadToSandbox: function (firstRun, afterUploadedCb) {
    //  return through(function (file, en, cb) {
    ...
    //      if (firstRun) {
    //        deleteAllAndUploadFiles();
    //      } else {
    //        uploadFiles(afterUploadedCb);
    //      }
    //    }, 1000);
    //    cb();
    //  });
