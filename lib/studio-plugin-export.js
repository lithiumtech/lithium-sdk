...
module.exports = function (gulp, gutil) {
  	...
  	function exportPlugin(doClear, verboseMode, server, pluginPointAnswers) {
		...
		return processResponse(req, doClear, useVerboseMode);
  	}

  	function processResponse(req, doClear, useVerboseMode) {
  	...
  	return {
    	exportPlugin: function (clear, verbose, server, answers) {
      		gutil.log(gutil.colors.cyan((clear ? 'Clearing' : 'Exporting') + ' plugin'));
      		
      		exportPlugin(clear, verbose, server, answers);
    	}
  	};
};