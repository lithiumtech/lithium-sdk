...
module.exports = function (gulp, gutil) {
  ...
  function processResponse(req, opts, cb) {
    ...
    req.on('response', function (resp) {
      if (resp.statusCode > 201) {
        return callbackOrThrowErrorMessage(cb, new Error(putils.logErrorResponseStatusCode(gutil, resp)));
      } else {
        res = resp;
        resp.pipe(stream);
      }
      ...
      return callbackOrThrowErrorMessage(cb, errorMsg);
    });
  ...
  function putOptions(server, opts, cb) {
    ...
    return handleRequest(server, opts, function(err) {
      ...
      opts.doPut = false;
      return handleRequest(server, opts, cb);
    });
  }
