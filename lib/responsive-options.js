...
module.exports = function (gulp, gutil) {
  ...
  function processResponse(req, opts, cb) {
    ...
    function handleResponse(res, buf, cb) {
      try {
        ...
        if (res.statusCode > 201) {
          ...
          writeError(respBody, 'responsive-' + opts.pluginType + '-plugin', cb);
    ...
    function writeResponsiveOptions(res, buf) {
      return handleResponse(res, buf, function(respBody) {
        ...
        fs.writeFile(responsiveConfigPath, JSON.stringify(responsiveConfig), function(err) {
          ...
          putils.logSuccess(gutil, successMsg);
          if (typeof cb !== 'undefined') {
            return cb(null, responsiveConfig);
          }
        });
      });
    }
    ...
    req.on('response', function (resp) {
      if (resp.statusCode > 201) {
        callbackOrThrowError(cb, new Error(putils.logErrorResponseStatusCode(gutil, resp)));
      } else {
        res = resp;
        resp.pipe(stream);
      }
  ...
  function putOptions(opts, cb) {
    ...
    opts.doPut = true;
    return handleRequest(opts, function(err) {
      if (err) throw err;
