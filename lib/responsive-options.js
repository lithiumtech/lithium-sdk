...
module.exports = function (gulp, gutil) {
  ...
  function handleRequest(server, opts, cb) {
    ...
    var callOpts = {};
    var pluginApiUrlBldr = putils.getPluginBaseUrl(gutil, server, opts, callOpts, useVerboseMode);
    var pluginApiUrl = pluginApiUrlBldr.build();
    var req;
  ...
  function isResponsiveSkin(config, skinDir) {
    var responsiveSkin = false;

    fs.readFileSync(path.resolve(skinDir, 'skin.properties')).toString().split('\n').forEach(function(line) {
      ...
      if (lineItem.length == 2) {
        ...
        if (key == 'parent') {
          ...
          //console.log('parent skin: ' + value);
          //console.log(config.skins.indexOf(value));
          responsiveSkin = config.skins.indexOf(value) > -1;
          //console.log("responsiveSkin: " + responsiveSkin);
        }
      }
    });
  ...
  function getResponsiveSkins(config) {
    var skins = [];
    skins.push.apply(skins, config.skins);
    var skinIds = fs.readdirSync('res/skins').filter(function(file) {
      var skinDir = path.resolve('res/skins', file);
      return fs.statSync(skinDir).isDirectory() && isResponsiveSkin(config, skinDir);
    });

    skinIds.forEach(function(item) {
    ...
    skins.push.apply(skins, skinIds);
      ...
      if ((gutil.env['force'] || server.force()) && !gutil.env['prompt']) {
        ...
        putOptions(server, {
          skinOpts: {
            enabled: true,
            id: skinId,
            url: 'http://localhost:' + port + '/skins/' + skinId,
          ...
          inquirer().prompt([
            {
              ...
              choices: getResponsiveSkins(config),
              default: function () {
            ...
            },
            {
              ...
              putOptions(server, {
                skinOpts: {
                  enabled: true,
                  id: answers.skinId,
                  url: 'http://localhost:' + answers.port + '/skins/' + answers.skinId,
