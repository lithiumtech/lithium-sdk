...
module.exports = function (gulp, gutil) {
  ...
  function handleRequest(server, opts, cb) {
    ...
    if (opts.doPut) {
      ...
      putils.logDebug(gutil, 'getOpts: ' + getOpts);
  ...
  function processResponse(req, opts, cb) {
    ...
    function getResponse(buf) {
      if (opts.debugMode) {
        putils.logDebug(gutil, 'response from responsive call: \n' + buf.toString());
      }
    ...
    req.on('response', function (res) {
      putils.logDebug(gutil, 'response: ' + res);
      var data = [], dataLen = 0;
      res.on('data', function(chunk) {
        putils.logDebug(gutil, 'data: ' + chunk);
        data.push(chunk);
  ...
  function getOptions(server, opts, cb) {
    ...
    if (opts.noCache || !fs.existsSync(configFilePath)) {
      handleRequest(server, opts, new function() {
        cb(getConfig(opts));
      });
    } else {
      cb(getConfig(opts));
