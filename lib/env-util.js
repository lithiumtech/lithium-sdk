...
var BOWER_COMPONENTS_PATH = sdkConf.newStructure ? 'angular-li/bower_components' : 'bower_components';
...
module.exports = function () {
    ...
    if (is.object(sdkConf.ng) && is.not.empty(sdkConf.ng)) {
        var directivePath = gutil.env.newStructure ? 'angular-li/src/directives' : 'src/directives';
        var servicesPath = gutil.env.newStructure ? 'angular-li/src/services' : 'src/services';
        gutil.env.ng = {
            module: sdkConf.ng.module || 'li',
            textProperties: getSourcePaths(sdkConf.ng.addTextProperties || [])
                .concat([directivePath, servicesPath, 'limuirs/src/components']),
