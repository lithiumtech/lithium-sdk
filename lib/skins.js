...
module.exports = function (gulp, gutil) {
  ...
  var assets = function (req, res, next) {
    ...
    var matches = assetUrl.match(/\/html\/(\S+)\/(\S+)/);
    ...
    var  assetPath = path().join( matches[1], matches[2]);
    ...
    var tmpDirPath = path().join(server.localServerDir(), "html", matches[1]);
   ...
   // var writeStream = fs().createWriteStream(tmpFilePath+".tmp");

    download(serverAssetUrl, tmpDirPath, function(){});
    next();
  }
