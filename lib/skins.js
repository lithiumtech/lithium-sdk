...
module.exports = function (gulp, gutil) {
  ...
  function createNewSkin(skinInfo) {
    ...
    function createSkinDirectoryStructure() {
        if (skinInfo.isResponsive) {
          ...
          const parentSkinSassDir = path.join(skinInfo.parentSkin.getDir(), skinLib.sassDir)
          if (fs().existsSync(parentSkinSassDir)) {
            const subDirs = fs().readdirSync(parentSkinSassDir)
              .filter(function (file) {
                return fs().statSync(path.join(parentSkinSassDir, file)).isDirectory();
            });
            subDirs.forEach(function(folder) {
              fs().mkdirSync(path.join(skinDir, skinLib.sassDir, folder), skinInfo.errorCb);
            });
          }
      ...
      return new Promise(function (resolve) {
        responsiveOptions.getOptions(server, opts, function (err, config) {
          ...
          opts.includePaths = [
            ...
            opts.includePathsPrefix + 'other/res/skins/',
            'res/skins'
          ];
  ...
  return {
    ...
    checkVersionForThemeSupport: checkVersionForThemeSupport
  };
};
