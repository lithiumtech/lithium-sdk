...
module.exports = function (gulp, gutil) {
  ...
  var baseFeaturePath = gutil.env.newStructure ? 'plugin/res/feature/responsivebase/' : 'res/feature/responsivebase/';
  var peakFeaturePath = gutil.env.newStructure ? 'plugin/res/feature/responsivepeak/' : 'res/feature/responsivepeak/';
  var themeFeaturePath = 'other/res/skins'
  ...
  function getLocalResponsiveSkins() {
    return localResponsiveSkins ? localResponsiveSkins
        : localResponsiveSkins = fs.readdirSync(skinsBaseDir).filter(function(file) {
      var skinDir = path.resolve(skinsBaseDir, file);
      return fs.statSync(skinDir).isDirectory();
    }).map(function(skinId) {
      var skinDir = path.resolve(skinsBaseDir, skinId);
  ...
  function getResponsiveFeatureVersionFromCore(featurePath, defaultPath) {
    ...
    if (!fs.existsSync(coreFeaturePath)) {
      putils.logWarning(gutil, 'No directory found under coreplugins for '+ featurePath + '. Returning default: ' + defaultPath);
      return defaultPath;
    }
  ...
  function findBaseDirForSkin(skinId) {
    ...
    if (getCoreResponsiveSkinIds().indexOf(skinId) > -1) {
      ...
      if (skinId.indexOf('responsive_peak') > -1) {
        return path.resolve(server.coreOutputDir(), peakFeaturePath,
          getResponsiveFeatureVersionFromCore(peakFeaturePath, server.localSkinCompileVersion()),
            skinsBaseDir, skinId);
      } else if (skinId.indexOf('responsive_base') > -1) {
        return path.resolve(server.coreOutputDir(), baseFeaturePath,
          getResponsiveFeatureVersionFromCore(baseFeaturePath, server.localSkinCompileVersion()),
  ...
  function Skin(id, dir) {
    ...
    function getSkinProperties(skin) {
        ...
        if (skin.isLocal()) {
          ...
          if (isFile(filePath)) {
            ...
            if (fileValByLines.length > 0) {
              ...
              // FIXME: line is not set at this point
              addSkinProp(line);
            }
          }
        } else {
    ...
    this.getIsTheme = function() {
      return isTheme;
    }

    this.setIsTheme = function(isThemeSkin) {
      isTheme = isThemeSkin;
    }
    ...
    async function checkParentTheme(parentId1) {
              var server = getServer();
              checkTheme = require('./check-theme-enabled.js')(gulp, gutil);
              await checkTheme.checkThemeEnabled(server.serverUrl(), server.pluginToken(), {
                          themeId: parentId1,
                          debugMode: gutil.env.debug
                        }, function(err, isThemeEnabled) {
                              if (err) {
                                process.exitCode = 1;
                                putils.logWarning(gutil, "ERROR");
                                throw new Error('Error checking enabled themes ' + err);
                              }
                              if(isThemeEnabled === 'true') {
                                parent = new Skin(parentId1, findBaseDirForSkin(parentId1));
                              } else {
                                parent = null;
                              }
                          }
      ...
      return getAttrib('responsive', function () {
        ...
        if (!responsiveSkin) {
          ...
          if (parent != null) {
            ...
            skin.setIsTheme(parent.getIsTheme());
          }
        }
  ...
  function setLiaVersion(liaVersion) {
      version = liaVersion;
  }
