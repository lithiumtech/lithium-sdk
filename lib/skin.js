  ...
  function findBaseDirForSkin(skinId) {
    ...
    if (getCoreResponsiveSkinIds().indexOf(skinId) > -1) {
      ...
      if (skinId.indexOf("responsive_peak") > -1) {
        return path.resolve(server.coreOutputDir(), peakFeaturePath, getResponsiveFeatureVersionFromCore(peakFeaturePath, server.localSkinCompileFeature()),
            skinsBaseDir, skinId);
      } else if (skinId.indexOf("responsive_base") > -1) {
        return path.resolve(server.coreOutputDir(), baseFeaturePath, getResponsiveFeatureVersionFromCore(baseFeaturePath, server.localSkinCompileFeature()),
