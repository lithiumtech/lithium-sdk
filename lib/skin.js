...
module.exports = function (gulp, gutil) {
  var server, localSkins, localResponsiveSkins;
  ...
  //TODO: Read this dynamically -- This list has been populated from 16.4 latest core skins
  ...
  function getLocalSkins() {
    return localSkins ? localSkins : fs.readdirSync(skinsBaseDir).filter(function(file) {
  ...
  function getLocalResponsiveSkins() {
    return localResponsiveSkins ? localResponsiveSkins : fs.readdirSync(skinsBaseDir).filter(function(file) {
      var skinDir = path.resolve(skinsBaseDir, file);
      return fs.statSync(skinDir).isDirectory();
    }).map(function(skinId) {
      var skinDir = path.resolve(skinsBaseDir, skinId);
  ...
  function findBaseDirForSkin(skinId) {
    ...
    if (getCoreResponsiveSkinIds().indexOf(skinId) > -1) {
      if (skinId.indexOf("responsive_peak") > -1) {
        return path.resolve(server.coreOutputDir(), peakFeaturePath, server.localSkinCompileVersion(),
            skinsBaseDir, skinId);
      } else if (skinId.indexOf("responsive_base") > -1) {
        return path.resolve(server.coreOutputDir(), baseFeaturePath, server.localSkinCompileVersion(),
