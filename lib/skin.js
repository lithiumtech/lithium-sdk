      ...
      if (!parent) {
        ...
        if (parentId) {
          if (parentId.includes("theme")) {
            const asyncFunction = (parentId1) => {
                return new Promise((resolve, reject) => {
                    var server = getServer();
                    checkTheme = require('./check-theme-enabled.js')(gulp, gutil);
                    checkTheme.checkThemeEnabled(server.serverUrl(), server.pluginToken(), {
                      themeId: parentId1,
                      debugMode: gutil.env.debug
                          ...
                          resolve(isThemeEnabled);
                      });
                })
              }
              
              const syncFunc = sp(asyncFunction)
              const value = syncFunc(parentId)
              if(value === 'true') {
