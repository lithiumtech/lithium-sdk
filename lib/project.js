  ...
  return {
    ...
    scaffold: function () {
      ...
      inquirer.prompt([
        ...
        //default plugin upload protocol
        ops.serverConf.pluginUploadProtocol = 'https';
        ...
        ops.description = 'li-sdk project: ' + answers.name + '\nGenerated by ' + process.env.USER;
        ...
        gulp.src(path.join(__dirname, '../templates/project/**/*'))
          ...
          .on('end', function () {
            ...
            fs.writeFile('./' + answers.name + '/server.conf.json', JSON.stringify(ops.serverConf, null, 2),
              function (error) {
                if (error) {
                  throw error;
                } else {
                  gutil.log(gutil.colors.cyan(path.resolve('./' + answers.name + '/server.conf.json')));

                  // npm link and install
                  npmLinkAndInstall('./' + answers.name, function (code) {
                    if (code === 0) {
    ...
    update: function (cb) {
      ...
      try {
        ops.serverConf = require(process.cwd() + '/server.conf.json');
      } catch (err) {
      ...
      inquirer.prompt([
        ...
        answers.name = process.cwd().split('/').pop();
        ops.name = answers.name;
        ops.description = 'li-sdk project: ' + answers.name + '\nGenerated by ' + process.env.USER;
        ...
        gulp.src([path.join(__dirname, '../templates/project/**/*'),
          ...
          .on('end', function () {
            // once templates are finished, create server.conf.json
            fs.writeFile('./server.conf.json', JSON.stringify(ops.serverConf, null, 2),
              function (error) {
                if (error) {
                  cb(error);
                } else {
